	...
	class Astra_Ext_Advanced_Hooks_Markup {
		...
		public function advanced_hook_scripts() {
			if ( is_singular( ASTRA_ADVANCED_HOOKS_POST_TYPE ) ) {
				...
				$padding_top    = astra_addon_sanitize_css_value( $padding_top, 'spacing' );
				$padding_bottom = isset( $padding['bottom'] ) ? $padding['bottom'] : '';
				$padding_bottom = astra_addon_sanitize_css_value( $padding_bottom, 'spacing' );
				...
				if ( ASTRA_ADVANCED_HOOKS_POST_TYPE !== $post_type ) {
					...
					add_action(
						...
						static function() use ( $post_id ) {
							...
							$wpb_custom_css = wp_strip_all_tags( get_post_meta( $post_id, '_wpb_shortcodes_custom_css', true ) );
