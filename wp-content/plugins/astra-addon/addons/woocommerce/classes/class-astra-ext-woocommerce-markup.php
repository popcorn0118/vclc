...
if ( ! class_exists( 'ASTRA_Ext_WooCommerce_Markup' ) ) {
 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function __construct() {
			...
			// WPML Multi-Currency compatibility for Quick View.
			add_filter( 'wcml_multi_currency_ajax_actions', array( $this, 'add_wpml_multi_currency_ajax_action' ) );
		...
		public function shop_js_localize( $localize ) {
			...
			$localize['quick_view_nonce']                = wp_create_nonce( 'ast_quick_view_nonce' );
		...
		public function previous_product_preview_image_insertion( $output, $format, $link, $post, $adjacent ) {
			if ( $post && $post->ID ) {
				...
				$image_markup .= '<div class="ast-navigation-content"><div class="ast-navigation-product-title">' . esc_html( get_the_title( $post->ID ) ) . '</div><div class="ast-navigation-price">' . wp_kses_post( $product->get_price_html() ) . '</div></div></div></div></a>';
	...
	<div class="product-links">
					...
					if ( ! $product->is_type( 'variable' ) ) {
						...
						foreach ( $product->get_children() as $child_id ) {
							...
							if ( $variation instanceof WC_Product ) {
								...
								if ( $sale_price ) {
									$regular_price                     = wc_format_decimal( $variation->get_regular_price() );
									$sale_price                        = wc_format_decimal( $sale_price );
									$percent_sale                      = round( ( $regular_price - $sale_price ) / $regular_price * 100, 0 );
									$sale_percent_value                = $sale_percent_value ? $sale_percent_value : '-[value]%';
									$text                              = str_replace( '[value]', $percent_sale, $sale_percent_value );
		...
		public function ast_load_product_quick_view_ajax() {
			...
			check_ajax_referer( 'ast_quick_view_nonce', 'nonce' );
		...
		/**
		 * Add Astra Quick View Ajax action to WPML Multi-Currency Ajax actions
		 *
		 * @param array $ajax_actions Array of Ajax action names that WPML should handle.
		 * @return array Modified array with Astra Quick View Ajax action added.
		 * @since 4.12.1
		 */
		public function add_wpml_multi_currency_ajax_action( $ajax_actions ) {

			if ( ! is_array( $ajax_actions ) ) {
				$ajax_actions = array();
			}
		...
		public function render_checkout_step_one_title() {
			return astra_get_i18n_option( 'two-step-checkout-modern-step-1-text', _x( '%astra%', 'WooCommerce Checkout: Step One Title', 'astra-addon' ) );
		}
		...
		public function render_checkout_step_one_sub_title() {
			return astra_get_i18n_option( 'two-step-checkout-modern-step-1-sub-text', _x( '%astra%', 'WooCommerce Checkout: Step One Sub Title', 'astra-addon' ) );
		}
		...
		public function render_checkout_step_two_title() {
			return astra_get_i18n_option( 'two-step-checkout-modern-step-2-text', _x( '%astra%', 'WooCommerce Checkout: Step Two Title', 'astra-addon' ) );
		}
		...
		public function render_checkout_step_two_sub_title() {
			return astra_get_i18n_option( 'two-step-checkout-modern-step-2-sub-text', _x( '%astra%', 'WooCommerce Checkout: Step Two Sub Title', 'astra-addon' ) );
		}
		...
		public function render_checkout_step_two_button_text() {
			return astra_get_i18n_option( 'two-step-checkout-modern-button-text', _x( '%astra%', 'WooCommerce Checkout: Offer Button Title', 'astra-addon' ) );
		}
		...
		public function render_checkout_step_two_button_sub_text() {
			return astra_get_i18n_option( 'two-step-checkout-modern-button-sub-text', _x( '%astra%', 'WooCommerce Checkout: Offer Button Sub Title', 'astra-addon' ) );
		}
		...
		public function render_checkout_place_order_text() {
			...
			return astra_get_i18n_option( 'checkout-place-order-text', _x( '%astra%', 'WooCommerce Checkout: Button Text (Place Order)', 'astra-addon' ) ) . $cart_total_with_symbol;
		}
		...
		public function render_checkout_two_step_form_note() {
			return astra_get_i18n_option( 'two-step-checkout-modern-note-text', _x( '%astra%', 'WooCommerce Checkout: Note Text', 'astra-addon' ) );
		}
		...
		public function render_checkout_payment_text() {
			return astra_get_i18n_option( 'checkout-payment-text', _x( '%astra%', 'WooCommerce Checkout: Payment Text', 'astra-addon' ) );
		}
		...
		public function render_checkout_customer_information_text() {
			return astra_get_i18n_option( 'checkout-customer-information-text', _x( '%astra%', 'WooCommerce Checkout: Customer information Text', 'astra-addon' ) );
		}
		...
		public function render_checkout_show_summary_text() {
			return astra_get_i18n_option( 'checkout-show-summary-text', _x( '%astra%', 'WooCommerce Checkout: Show Summary Text', 'astra-addon' ) );
		}
		...
		public function render_checkout_hide_summary_text() {
			return astra_get_i18n_option( 'checkout-hide-summary-text', _x( '%astra%', 'WooCommerce Checkout: Hide Summary Text', 'astra-addon' ) );
		}
		...
		public function render_coupon_text() {
			return astra_get_i18n_option( 'woo-coupon-text', _x( '%astra%', 'WooCommerce Misc: Coupon Text', 'astra-addon' ) );
		}
		...
		public function render_coupon_apply_text() {
			return astra_get_i18n_option( 'woo-coupon-apply-text', _x( '%astra%', 'WooCommerce Misc: Coupon Apply Text', 'astra-addon' ) );
		}
