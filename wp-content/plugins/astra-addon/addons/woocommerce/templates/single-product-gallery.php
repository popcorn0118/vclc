...
function get_gallery_thumbnail( $attachment_id, $slide_number ) {
	...
	$thumbnail_url = $thumbnail_src ? $thumbnail_src[0] : '';
	if ( empty( $thumbnail_url ) || strpos( $thumbnail_url, '-100x100' ) !== false ) {
		$thumbnail_src = wp_get_attachment_image_src( $attachment_id, 'woocommerce_thumbnail' );
		$thumbnail_url = $thumbnail_src ? $thumbnail_src[0] : '';
	}
	$original_url = $original_src ? $original_src[0] : '';
	$image        = wp_get_attachment_image( $attachment_id, $thumbnail_size, false, array( 'data-original-src' => $original_url ) );
	if ( empty( $image ) ) {
		return '';
	}
	...
	return '<div data-slide-number="' . esc_attr( $slide_number ) . '" data-thumb="' . esc_url( $thumbnail_url ) . '" data-thumb-alt="' . esc_attr( $alt_text ) . '" class="ast-woocommerce-product-gallery__image ' . esc_attr( $is_first_slide ) . '">' . $image . '</div>';
}
