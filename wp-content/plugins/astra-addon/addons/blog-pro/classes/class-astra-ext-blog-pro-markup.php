...
if ( ! class_exists( 'Astra_Ext_Blog_Pro_Markup' ) ) {
	...
	class Astra_Ext_Blog_Pro_Markup {
		...
		public function __construct() {
			...
			add_filter( 'astra_blog_post_thumb_img_content_after', array( $this, 'category_inside_image' ) );
			add_filter( 'astra_skip_blog_archive_category', array( $this, 'should_skip_category_output' ) );
			...
			add_filter( 'posts_orderby', array( $this, 'stabilize_blog_pagination_orderby' ), 10, 2 );
		}
		...
		public function enqueue_frontend_scripts() {
			...
			if ( astra_get_option( 'ast-auto-prev-post' ) && is_single() ) {
		...
		/**
		 * Stabilize ORDER BY for blog pagination queries.
		 *
		 * Adds ID DESC as a tiebreaker when posts share the same post_date,
		 * preventing duplicate posts across infinite scroll pages.
		 *
		 * @since 4.13.2
		 *
		 ...
		 */
		public function stabilize_blog_pagination_orderby( $orderby, $query ) {
			// Skip secondary/widget queries; only stabilize the main loop.
			if ( ! $query->is_main_query() ) {
				return $orderby;
			}

			// Only relevant on blog index, archive, and search result pages.
			if ( ! $query->is_home() && ! $query->is_archive() && ! $query->is_search() ) {
				return $orderby;
			}

			// Only applies when infinite scroll pagination is active.
			if ( 'infinite' !== astra_get_option( 'blog-pagination' ) ) {
				return $orderby;
			}

			// Only applies to standard posts, not CPT or WooCommerce archives.
			$queried_post_type = $query->get( 'post_type' );
			if ( ! empty( $queried_post_type ) && 'post' !== $queried_post_type ) {
				return $orderby;
			}

			global $wpdb;
			$post_date_col = $wpdb->posts . '.post_date DESC';
			$post_id_col   = $wpdb->posts . '.ID';

			if ( false !== strpos( $orderby, $post_date_col ) && false === strpos( $orderby, $post_id_col ) ) {
				$orderby .= ', ' . $post_id_col . ' DESC';
			}
		...
		/**
		 * Display categories inside the featured image container for specific blog layouts.
		 *
		 * @since 4.12.0
		 * @return string HTML markup for categories or empty string if conditions are not met.
		 */
		public function category_inside_image() {
			$blog_layout               = astra_addon_get_blog_layout();
			...
			if ( in_array( $blog_layout, array( 'blog-layout-4', 'blog-layout-5' ) ) && ( $blog_category_alignment_h || $blog_category_alignment_v ) && ! is_singular() ) {
				return astra_post_categories( 'astra_blog_archive_category', 'blog-category-style', true );
			}
		...
		/**
		 * Skip category output when categories are positioned inside image container.
		 *
		 * @since 4.12.0
		 * @param bool $skip Whether to skip category output.
		 * @return bool Whether to skip category output.
		 */
		public function should_skip_category_output( $skip ) {
			if ( is_singular() ) {
				return $skip;
			}
			...
			if ( in_array( $blog_layout, array( 'blog-layout-4', 'blog-layout-5' ) ) && ( $blog_category_alignment_h || $blog_category_alignment_v ) ) {
				return true;
			}
