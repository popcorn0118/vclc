...
function astra_ext_blog_pro_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {
	...
	if ( $blog_card_alignment_h || $blog_card_alignment_v ) {
		...
		$css_output['.post-content .ast-blog-featured-section'] = array(
			'align-self' => 'stretch',
			'width'      => '100%',
		);

		$effective_v = $blog_card_alignment_v ? $blog_card_alignment_v : 'middle';

		if ( 'top' !== $effective_v ) {
			$css_output['.post-content > *:first-child'] = array(
				'margin-top' => 'auto',
			);

			if ( 'middle' === $effective_v ) {
				$css_output['.post-content > *:last-child'] = array(
					'margin-bottom' => 'auto',
				);
			}

			// Pin featured image to top when it's the first child.
			$css_output['.post-content > .ast-blog-featured-section:first-child']     = array(
				'margin-top' => '0',
			);
			$css_output['.post-content > .ast-blog-featured-section:first-child + *'] = array(
				'margin-top' => 'auto',
		...
		if ( $inherit_card_style ) {
			...
			$css_output['.ast-related-post-content .ast-blog-featured-section'] = array(
				'align-self' => 'stretch',
				'width'      => '100%',
			);

			if ( 'top' !== $effective_v ) {
				$css_output['.ast-related-post-content > *:first-child'] = array(
					'margin-top' => 'auto',
				);

				if ( 'middle' === $effective_v ) {
					$css_output['.ast-related-post-content > *:last-child'] = array(
						'margin-bottom' => 'auto',
					);
				}

				$css_output['.ast-related-post-content > .ast-blog-featured-section:first-child']     = array(
					'margin-top' => '0',
				);
				$css_output['.ast-related-post-content > .ast-blog-featured-section:first-child + *'] = array(
					'margin-top' => 'auto',
