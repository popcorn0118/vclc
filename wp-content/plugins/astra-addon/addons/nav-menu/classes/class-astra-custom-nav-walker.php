 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( 0 === $depth ) {
				...
				$this->megamenu_trigger = $this->get_post_meta( $post_meta, '_menu_item_megamenu_trigger' );
				$this->megamenu_trigger = ! empty( $this->megamenu_trigger ) ? $this->megamenu_trigger : 'hover';
			...
			// Add data-megamenu-trigger attribute for mega menu items.
			$data_attrs = '';
			if ( 0 === $depth && $this->has_children && '' != $this->megamenu && 'ast-hf-mobile-menu' !== $args->menu_id && 'ast-desktop-toggle-menu' !== $args->menu_id ) {
				$data_attrs = ' data-megamenu-trigger="' . esc_attr( $this->megamenu_trigger ) . '"';
			}
