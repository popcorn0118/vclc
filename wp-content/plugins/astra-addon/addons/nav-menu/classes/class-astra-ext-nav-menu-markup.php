...
if ( ! class_exists( 'Astra_Ext_Nav_Menu_Markup' ) ) {
 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function set_mega_menu_option( $req ) {
			...
			$menu_id = isset( $req['menu_id'] ) ? absint( $req['menu_id'] ) : 0;
			$widgets = isset( $req['widgets'] ) ? $req['widgets'] : array();

			if ( ! current_user_can( 'edit_theme_options' ) ) {
				return new WP_Error( 'rest_forbidden', __( 'Permission denied.', 'astra-addon' ), array( 'status' => 403 ) );
			}

			// Validate menu_id is a nav_menu_item post type to prevent arbitrary post meta writes.
			if ( empty( $menu_id ) || 'nav_menu_item' !== get_post_type( $menu_id ) ) {
				return new WP_Error( 'invalid_menu_item', __( 'Invalid menu item ID.', 'astra-addon' ), array( 'status' => 400 ) );
			}
			...
			if ( ! empty( $fields ) ) {
				...
				foreach ( $fields as $key => $value ) {
					...
					if ( 'megamenu_custom_text' === $key ) {
						...
						$value = astra_addon_sanitize_recursive( wp_unslash( $value ) );
					}
		...
		public function get_mega_menu_option( $data ) {
			...
			$_config = array(
				...
				// Option: Mega Menu Trigger Behavior.
				array(
					'name'     => ASTRA_THEME_SETTINGS . '_menu_item_megamenu_trigger',
					'defaults' => get_post_meta( $menu_item_id, '_menu_item_megamenu_trigger', true ) ? get_post_meta( $menu_item_id, '_menu_item_megamenu_trigger', true ) : 'hover',
					'control'  => 'ast-select',
					'title'    => __( 'Open Mega Menu On', 'astra-addon' ),
					'divider'  => '',
					'choices'  => array(
				...
				array(
					...
					'tab_type'        => 'general',
					'depth'           => 'all',
					'custom_value'    => 'has-label',
					'trigger_context' => array(
						array(
							'setting' => ASTRA_THEME_SETTINGS . '_menu_item_highlight_labels_title',
							'value'   => array( 'has-label' ),
						),
						array(
							'setting' => ASTRA_THEME_SETTINGS . '_menu_item_megamenu_label_color',
							'value'   => array( 'has-label' ),
						),
						array(
							'setting' => ASTRA_THEME_SETTINGS . '_menu_item_megamenu_label_bg_color',
							'value'   => array( 'has-label' ),
						),
					),
				),
				...
				array(
					...
					'context'  => array(
						array(
							'value'        => array( 'has-label' ),
							'target_value' => get_post_meta( $menu_item_id, '_menu_item_megamenu_highlight_label', true ) ? 'has-label' : '',
						),
					),
				),
				...
				array(
					...
					'control'  => 'ast-color',
					'title'    => __( 'Highlight Label Background Color', 'astra-addon' ),
					'divider'  => '',
					'tab_type' => 'design',
					'depth'    => 'all',
					'context'  => array(
