...
class Astra_Addon_Admin_Loader {
	...
	public function includes() {
		if ( is_admin() ) {
			...
			require_once ASTRA_ADDON_ADMIN_DIR . 'includes/class-astra-addon-license-notice.php';
		}
	...
	public function styles_scripts() {
		...
		$localize = array(
			...
			'license_expires'                      => ASTRA_ADDON_BSF_PACKAGE && ! empty( $product_id ) && function_exists( 'bsf_get_license_expires' ) ? bsf_get_license_expires( $product_id ) : '',
			'product'                              => 'astra-addon',
					...
					<div class="max-w-3xl mx-auto lg:max-w-full h-[62px]">
						<div class="relative flex flex-wrap flex-col sm:flex-row justify-between items-start sm:items-center h-full min-h-24 sm:min-h-15 pt-14 sm:pb-0 sm:pt-0">
							<div class="flex items-center gap-6">
								...
								<h5 class="text-2xl [8px] my-0 mr-3 pl-6 border-l-[1px] border-slate-200 font-semibold tracking-[-0.006em] align-middle table-view-header"><?php echo esc_html( $title ); ?></h5>
