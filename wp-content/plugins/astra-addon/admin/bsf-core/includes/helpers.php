	...
	function bsf_extension_nag( $product_id = '', $mu_updater = false ) {
		...
		if ( '' !== $bsf_not_activated_plugins || '' !== $bsf_not_installed_plugins ) {
			...
			if ( '' !== $bsf_not_activated_plugins ) {
				echo '<p>';
				echo esc_html( $bsf_product_name ) . esc_html__( ' requires following plugins to be active : ', 'astra-addon' );
				echo '<strong><em>';
			...
			if ( '' !== $bsf_not_installed_plugins ) {
				echo '<p>';
				echo esc_html( $bsf_product_name ) . esc_html__( ' requires following plugins to be installed and activated : ', 'astra-addon' );
				echo '<strong><em>';
			...
			if ( true === $bsf_activate ) {
				$installer .= '<a href="' . get_admin_url() . 'plugins.php?plugin_status=inactive">' . __( 'Begin activating plugins', 'astra-addon' ) . '</a> | ';
			}

			if ( true === $bsf_install ) {
				$installer .= '<a href="' . bsf_exension_installer_url( $product_id ) . '">' . __( 'Begin installing plugins', 'astra-addon' ) . '</a> | ';
			}

			$installer .= '<a href="' . esc_url( add_query_arg( 'bsf-dismiss-notice', $product_id ) ) . '">' . __( 'Dismiss This Notice', 'astra-addon' ) . '</a>';
...
if ( ! function_exists( 'bsf_nag_brainstorm_updater_multisite' ) ) {
	...
	function bsf_nag_brainstorm_updater_multisite() {
		...
		sprintf( __( 'Looks like you are on a WordPress Multisite, you will need to install and network activate %1$s Brainstorm Updater for Multisite %2$s plugin. Download it from %3$s here %4$s', 'astra-addon' ), '<strong><em>', '<strong><em>', '<a href="http://bsf.io/bsf-updater-mu" target="_blank">', '</a>' );
...
if ( ! function_exists( 'bsf_get_license_expires' ) ) {
	 ...
	 *
	 * @param string $product_id Product ID.
	 * @return string Date string (Y-m-d H:i:s), 'lifetime', or empty string when not set.
	 */
...
if ( ! function_exists( 'bsf_is_license_expired' ) ) {
...
if ( ! function_exists( 'bsf_is_license_expiring_soon' ) ) {
	 ...
	 *
	 * @param string    $product_id Product ID.
	 * @param int|float $days       Threshold in days (default 14); non-integer values are cast to int.
	 * @return bool
	 */
