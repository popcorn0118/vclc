...
function bsf_render_bundled_products( $product_id, $installed ) {
	...
	if ( $total_bundled_plugins === $global_plugin_installed ) {
		...
		<div class="bsf-extensions-no-active">
			...
			<p class="bsf-text-light"><em><?php esc_html_e( 'All available extensions have been installed!', 'astra-addon' ); ?></em></p>
		</div>
		<?php
		return ob_get_clean();
	}
	...
	if ( empty( $brainstrom_bundled_products ) ) {
		...
		<div class="bsf-extensions-no-active">
			...
			<div class="bsf-cp-rem-bundle" style="margin-top: 30px;">
				<a class="button-primary" href="<?php echo esc_url( $reset_bundled_url ); ?>"><?php esc_html_e( 'Refresh Bundled Products', 'astra-addon' ); ?></a>
			</div>
		</div>
		<?php
		return ob_get_clean();
			...
			<?php if ( ! $is_plugin_installed ) { ?>
				<div class="bsf-extension-start-install">
					<div class="bsf-extension-start-install-content">
						<h2><?php esc_html_e( 'Downloading', 'astra-addon' ); ?><div class="bsf-css-loader"></div></h2>
					</div>
				</div>
			<?php } ?>
			...
			<div class="bottom-section">
				...
				if ( ! $is_plugin_installed ) {
					if ( ( ! $product->licence_require || 'false' === $product->licence_require ) || 'registered' === $product_status ) {
						...
						$button        = __( 'Validate Purchase', 'astra-addon' );
					...
					$button = __( 'Installed', 'astra-addon' );
				}
