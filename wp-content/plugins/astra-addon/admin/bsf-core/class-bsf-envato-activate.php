...
class BSF_Envato_Activate {
	...
	public function envato_register( $args ) {
		...
		$button_text_activate    = ( isset( $args['button_text_activate'] ) && ! is_null( $args['button_text_activate'] ) ) ? __( 'Sign Up & Activate', 'astra-addon' ) : __( 'Sign Up & Activate', 'astra-addon' );
		...
		if ( $is_active ) {
			...
			$envato_not_active_oauth_title    = apply_filters( "envato_not_active_oauth_title_{$product_id}", __( 'Updates & Support Registration - <span class="not-active">Not Active!</span>', 'astra-addon' ) );
			$envato_not_active_oauth_subtitle = apply_filters( "envato_not_active_oauth_subtitle_{$product_id}", __( 'Click on the button below to activate your license and subscribe to our newsletter.', 'astra-addon' ) );
