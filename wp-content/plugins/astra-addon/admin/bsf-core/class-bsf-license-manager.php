...
if ( ! class_exists( 'BSF_License_Manager' ) ) {
	...
	class BSF_License_Manager {
		 ...
		 *
		 * @since 1.29.14 Clears stored `expires` on successful deactivation.
		...
		public function process_license_deactivation( $product_id ) {
			...
			if ( ! is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) === 200 ) {
				...
				if ( isset( $result['success'] ) && ( true === $result['success'] || 'true' === $result['success'] ) ) {
					...
					// Deactivation response never includes expires — set it explicitly so the merge clears the stored value.
					$result['expires'] = '';
		...
		public function bsf_update_product_info( $product_id, $args ) {
			...
			foreach ( $brainstrom_products as $type => $products ) {
				...
				foreach ( $products as $id => $product ) {
					...
					if ( $id == $product_id ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
		 ...
		 *
		 * @since 1.29.14 Stores `expires` field from API response into product info.
		...
		public function get_remote_license_status( $purchase_key, $product_id ) {
			...
			if ( ! is_wp_error( $response ) && wp_remote_retrieve_response_code( $response ) === 200 ) {
				...
				// Store expiry date if returned by the API (empty string clears stale values for inactive licenses).
				if ( isset( $response_body['expires'] ) ) {
					$this->bsf_update_product_info(
						$product_id,
						array( 'expires' => sanitize_text_field( $response_body['expires'] ) )
					);
				}
			}
		...
		public function license_activation_form( $args ) {
			...
			if ( empty( $product_id ) ) {
				esc_html_e( 'Product id is missing.', 'astra-addon' );
			...
			$button_text_activate    = ( isset( $args['button_text_activate'] ) && ! is_null( $args['button_text_activate'] ) ) ? $args['button_text_activate'] : __( 'Activate License', 'astra-addon' );
			$button_text_deactivate  = ( isset( $args['button_text_deactivate'] ) && ! is_null( $args['button_text_deactivate'] ) ) ? $args['button_text_deactivate'] : __( 'Deactivate License', 'astra-addon' );
			...
			if ( false === $is_active || 'false' === $is_active ) {
				...
				if ( true === $bsf_license_allow_email || 'true' === $bsf_license_allow_email ) {
					$popup_license_subtitle = apply_filters( "bsf_license_key_form_inactive_subtitle_{$product_id}", sprintf( '<p>%s</p>', __( 'Click on the button below to activate your license and subscribe to our newsletter.', 'astra-addon' ) ) );
				} else {
					$popup_license_subtitle = apply_filters( "bsf_license_key_form_inactive_subtitle_{$product_id}", sprintf( '<p>%s</p>', __( 'Enter your purchase key and activate automatic updates.', 'astra-addon' ) ) );
				}
			} else {
			...
			if ( true === $is_active || 'true' === $is_active ) {
				...
				$licnse_active_message = __( 'Your license is active.', 'astra-addon' );
		...
		public function get_bsf_inline_license_form( $links, $args, $license_from_type ) {
			...
			$status         = 'inactive';
			$license_string = __( 'Activate License', 'astra-addon' );
			if ( self::bsf_is_active_license( $product_id ) ) {
				$status         = 'active';
				$license_string = __( 'License', 'astra-addon' );
			}
				...
				<div plugin-slug="<?php echo esc_attr( $product_id ); ?>" class="bsf-core-license-form" style="display: none;">
					...
					<div class="bsf-core-license-form-inner">
						<button type="button" class="bsf-core-license-form-close-btn">
							<span class="screen-reader-text"><?php esc_html_e( 'Close', 'astra-addon' ); ?></span>
