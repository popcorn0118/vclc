...
class BSF_Rollback_Version_Manager {
	...
	public function render_rollback_version_form( $product_id ) {
		...
		<div class="bsf-rollback-version">
			...
			<a data-placeholder-text=" <?php echo esc_html__( 'Rollback', 'astra-addon' ); ?>" href="<?php echo esc_url( add_query_arg( 'version_no', $sorted_versions[0], wp_nonce_url( admin_url( 'index.php?action=bsf_rollback&product_id=' . $product_id ), 'bsf_rollback' ) ) ); ?>"
			data-placeholder-url="<?php echo esc_url( wp_nonce_url( admin_url( 'index.php?action=bsf_rollback&version_no=VERSION&product_id=' . $product_id ), 'bsf_rollback' ) ); ?>" class="button bsf-rollback-button"><?php echo esc_html__( 'Rollback', 'astra-addon' ); ?> </a>
		</div>
		<?php
	}
	...
	public function rollback_version_popup() {
		...
		<div class="bsf-confirm-rollback-popup" style="display:none;">
			...
			<div class="bsf-confirm-rollback-popup-content">
				<h3 class="bsf-rollback-heading bsf-confirm-heading" data-text="<?php esc_html_e( 'Rollback #PRODUCT_NAME# Version', 'astra-addon' ); ?>"></h3>
				<p class="bsf-confirm-text bsf-rollback-text" data-text="<?php esc_html_e( 'Are you sure you want to rollback #PRODUCT_NAME# to version #VERSION#?', 'astra-addon' ); ?>" ></p>
				<div class="bsf-confirm-rollback-popup-buttons-wrapper">
					<button class="button bsf-product-license button-default bsf-confirm-cancel"><?php esc_html_e( 'Cancel', 'astra-addon' ); ?></button>
					<button class="button button-primary  bsf-confirm-ok"><?php esc_html_e( 'Continue', 'astra-addon' ); ?></button>
				</div>
			</div>
		</div>
		<?php
