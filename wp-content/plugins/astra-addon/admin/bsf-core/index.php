...
if ( ! defined( 'BSF_UPDATER_PATH' ) ) {
	define( 'BSF_UPDATER_PATH', __DIR__ );
}
	...
	function register_bsf_extension_page_network() {
		...
		foreach ( $themes as $theme ) {
			if ( 'iMedica' === $theme->Name ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				add_submenu_page( $parent_slug, __( 'iMedica Extensions', 'astra-addon' ), __( 'iMedica Extensions', 'astra-addon' ), 'manage_options', 'bsf-extensions-10395942', 'bsf_extensions_callback' );
				break;
			}
		}
	}
...
if ( ! function_exists( 'bsf_registration_page_url' ) ) {
	...
	function bsf_registration_page_url( $append = '', $product_id = '' ) {
		...
		if ( defined( 'BSF_REG_MENU_TO_SETTINGS' ) && ( true === BSF_REG_MENU_TO_SETTINGS || 'true' === BSF_REG_MENU_TO_SETTINGS ) ) {
			$constant = true;
		}
		...
		} elseif ( true === $option || true === $constant ) {
...
if ( ! function_exists( 'bsf_set_options' ) ) {
	...
	function bsf_set_options() {
		...
		if ( isset( $_GET['reset-bsf-users'] ) ) {
			...
			check_admin_referer( 'bsf-reset-users' );
		...
		if ( isset( $_GET['remove-bundled-products'] ) ) {
			...
			check_admin_referer( 'bsf-remove-bundled-products' );
