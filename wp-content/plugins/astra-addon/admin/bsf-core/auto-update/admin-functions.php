	...
	function bsf_notices() {
		...
		if ( 'update-core.php' === $pagenow || 'plugins.php' === $pagenow || 'post-new.php' === $pagenow || 'edit.php' === $pagenow || 'post.php' === $pagenow ) {
			...
			foreach ( $mix as $product ) :
				...
				if ( 'registered' !== $status && isset( $product['product_name'] ) ) :

					$url = bsf_registration_page_url( '', $product['id'] );

					$message = __( 'Please', 'astra-addon' ) . ' <a href="' . esc_url( $url ) . '" class="bsf-core-license-form-btn" plugin-slug="' . esc_html( $product['id'] ) . '">' . __( 'activate', 'astra-addon' ) . '</a> ' . __( 'your copy of the', 'astra-addon' ) . ' <i>' . esc_html( $product['product_name'] ) . '</i> ' . __( 'to get update notifications, access to support features & other resources!', 'astra-addon' );
