	...
	class BSF_Analytics {
				...
				BSF_Admin_Notices::add_notice(
					array(
						...
						'message'                    => sprintf(
							...
							sprintf( $notice_string . '<span dir="%1s"> <a href="%2s" target="_blank" rel="noreferrer noopener">%3s</a><span><br><br>', $language_dir, esc_url( $usage_doc_link ), __( 'Learn more.', 'astra-addon' ) ),
							esc_url(
								add_query_arg(
									array(
		...
		public function render_settings_field_html( $args ) {
			...
			<label for="<?php echo esc_attr( $args['label_for'] ); ?>">
				...
				echo esc_html( sprintf( __( 'Help improve %s by sharing non-sensitive usage data — like PHP version and features used.', 'astra-addon' ), $args['title'] ) );// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText

				if ( is_multisite() ) {
					esc_html_e( ' This will be applicable for all sites from the network.', 'astra-addon' );
				}
				?>
			</label>
			<?php
