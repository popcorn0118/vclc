	...
	class Deactivation_Survey_Feedback {
		...
		public static function show_feedback_form( array $args = array() ) {
			...
			$defaults = array(
				...
				'popup_description' => __( 'If you have a moment, please share why you are deactivating the plugin.', 'astra-addon' ),
				...
				<div class="uds-feedback-form--container">
					<div class="uds-form-header--wrapper">
						<div class="uds-form-title--icon-wrapper">
							<?php if ( ! empty( $args['popup_logo'] ) ) { ?>
								<img class="uds-icon" src="<?php echo esc_url( $args['popup_logo'] ); ?>" title="<?php echo esc_attr( $args['plugin_slug'] ); ?> <?php echo esc_attr( __( 'Icon', 'astra-addon' ) ); ?>" />
							<?php } ?>
					...
					<div class="uds-form-body--content">
						...
						<?php foreach ( $args['popup_reasons'] as $key => $value ) {
							...
							<fieldset>
								<textarea class="uds-options-feedback hide" id="uds-options-feedback" rows="3" name="uds_options_feedback" placeholder="<?php echo esc_attr( __( 'Please tell us more details.', 'astra-addon' ) ); ?>"></textarea>
								<?php
										...
										<p class="uds-option-feedback-cta hide">
											...
											echo wp_kses_post(
												...
												/* translators: %1$s: link html start, %2$s: link html end*/
													__( 'Need help from our experts? %1$sClick here to contact us.%2$s', 'astra-addon' ),
							...
							<div class="uds-feedback-form-sumbit--actions">
								<button class="button button-primary uds-feedback-submit" data-action="submit"><?php esc_html_e( 'Submit & Deactivate', 'astra-addon' ); ?></button>
								<button class="button button-secondary uds-feedback-skip" data-action="skip"><?php esc_html_e( 'Skip & Deactivate', 'astra-addon' ); ?></button>
		...
		public function send_plugin_deactivate_feedback() {
			...
			$response_data = array( 'message' => __( 'Sorry, you are not allowed to do this operation.', 'astra-addon' ) );
			...
			if ( ! check_ajax_referer( 'uds_plugin_deactivate_feedback', 'security', false ) ) {
				$response_data = array( 'message' => __( 'Nonce validation failed', 'astra-addon' ) );
				wp_send_json_error( $response_data );
			}
