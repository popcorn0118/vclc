...
/**
 * Sanitize a CSS value based on the expected type.
 *
 * @param string $value The raw value to sanitize.
 * @param string $type  The type of CSS value. Accepts 'spacing'.
 *                      Default 'spacing'.
 *
 ...
 * @since 4.12.5
 */
function astra_addon_sanitize_css_value( $value, $type = 'spacing' ) {
	...
	switch ( $type ) {
		case 'spacing':
			...
			return preg_match( '/^\d*\.?\d+(px|em|rem|%|vh|vw)$/', $value ) ? $value : '';
			break;
