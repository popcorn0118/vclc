 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function astra_get_posts_by_query() {
			...
			if ( ! current_user_can( 'edit_posts' ) ) {
				wp_send_json_error( array( 'message' => __( 'Permission denied.', 'astra-addon' ) ) );
			}
		...
		public function get_posts_by_conditions( $post_type, $option ) {
			...
			if ( false === $meta_header ) {
				...
				$raw_post_type     = function_exists( 'astra_get_post_type' ) ? astra_get_post_type() : get_post_type();
				$current_post_type = esc_sql( $wpdb->esc_like( $raw_post_type ) );
				...
				if ( class_exists( 'SitePress' ) ) {
					...
					if ( false != $cpt_translation_mode ) {
						...
						$wpml_translate_query_condition = "AND icl.language_code = '" . esc_sql( $current_language ) . "'";
					}
				}
				 ...
				 *
				 * SECURITY: $meta_args is interpolated directly into a wpdb::prepare() query string,
				 * so any values appended here MUST be escaped before use:
				 ...
				 *
				 * @param string          $meta_args       SQL fragment (AND/OR conditions, no leading AND/OR).
				 * @param WP_Term|WP_Post|null $q_obj      Current queried object.
				 * @param int|false       $current_post_id Current post ID, or false if not yet resolved.
				 */
				...
				switch ( $current_page_type ) {
					...
					case 'is_author':
						...
						if ( 'is_tax' === $current_page_type && ( is_category() || is_tag() || is_tax() ) ) {
							...
							if ( is_object( $q_obj ) ) {
								$meta_args .= " OR pm.meta_value LIKE '%\"" . $current_post_type . '|all|taxarchive|' . esc_sql( $wpdb->esc_like( $q_obj->taxonomy ) ) . "\"%'";
								$meta_args .= " OR pm.meta_value LIKE '%\"tax-" . absint( $q_obj->term_id ) . "\"%'";
							}
					...
					case 'is_singular':
						...
						if ( $check_wpml ) {
							$current_id = icl_object_id( $current_id, $raw_post_type, true, $current_language );
						}
						...
						if ( $current_post_obj && $current_post_obj->post_parent ) {
							...
							if ( ! empty( $parent_ids ) ) {
								foreach ( $parent_ids as $pid ) {
									$meta_args .= " OR (pm.meta_value LIKE '%\"specifics\"%' AND pm.meta_value LIKE '%\"apply_on_child\";b:1%' AND pm.meta_value LIKE '%\"post-" . absint( $pid ) . "\"%')";
								}
							}
						}
						...
						if ( is_object( $q_obj ) ) {
							...
							foreach ( $terms as $term ) {
								$meta_args .= " OR pm.meta_value LIKE '%\"tax-" . absint( $term->term_id ) . '-single-' . esc_sql( $wpdb->esc_like( $term->taxonomy ) ) . "\"%'";
							}
						}
				...
				// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $meta_args is built with esc_sql/esc_like/absint in this method and is filterable via 'astra_addon_meta_args_post_by_condition' (filter consumers are responsible for escaping). $wpml_translate_query and $wpml_translate_query_condition use esc_sql/trusted wpdb properties.
				$posts = $wpdb->get_results( $wpdb->prepare( "SELECT p.ID, pm.meta_value FROM {$wpdb->postmeta} as pm INNER JOIN {$wpdb->posts} as p ON pm.post_id = p.ID {$wpml_translate_query} WHERE pm.meta_key = %s AND p.post_type = %s {$wpml_translate_query_condition} AND p.post_status = 'publish' AND ({$meta_args}) ORDER BY p.post_date DESC", $location, $post_type ) );
